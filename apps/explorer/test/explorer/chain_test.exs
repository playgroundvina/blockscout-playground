defmodule Explorer.ChainTest do
  ...
  describe "block_reward_by_parts/1" do
    ...
    test "without uncles", %{emission_reward: %{reward: reward, block_range: range}} do
      block = build(:block, number: range.from, base_fee_per_gas: %Wei{value: Decimal.new(5)})
    ...
    test "with uncles", %{emission_reward: %{reward: reward, block_range: range}} do
      block = build(:block, number: range.from)
      uncle_block = insert(:block)
      insert(:block_second_degree_relation, nephew: block, uncle_hash: uncle_block.hash)
